...
echo -e "\e[1;32mDave, I'm updating the package list...\e[0m" && apt-get -qq update > /dev/null && apt-get -qq install apt-transport-https whiptail locales > /dev/null
...
for halBox_package in ack-grep bc bcrypt build-essential cloc curl dialog dstat glances host htop httpie iftop ioping iotop jpegoptim nano ncdu optipng pydf scrypt ssdeep strace tmux units unzip vim virt-what zip; do
    echo -e "\e[1;32mDave, I'm installing '$halBox_package'.\e[0m" && apt-get -qq install $halBox_package > /dev/null 2>&1
done
