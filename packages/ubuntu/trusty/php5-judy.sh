...
if [[ ! -f /etc/php5/mods-available/judy.ini ]]; then
	...
	pear config-set php_ini /etc/php5/mods-available/judy.ini > /dev/null && printf "\n" | pecl -q install -f judy > /dev/null

	if [[ $(cat /etc/php5/mods-available/judy.ini | wc -c) -le 1 ]]; then
		echo -e "\e[1;31mSomething went wrong installing '$halBox_PHP_package'.\e[0m" && rm -f /etc/php5/mods-available/judy.ini
	fi
