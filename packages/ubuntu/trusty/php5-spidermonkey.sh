...
if [[ ! -f /etc/php5/mods-available/spidermonkey.ini ]]; then
	...
	pear config-set php_ini /etc/php5/mods-available/spidermonkey.ini > /dev/null && printf "\n" | pecl -q install -f spidermonkey > /dev/null

	if [[ $(cat /etc/php5/mods-available/spidermonkey.ini | wc -c) -le 1 ]]; then
		echo -e "\e[1;31mSomething went wrong installing '$halBox_PHP_package'.\e[0m" && rm -f /etc/php5/mods-available/spidermonkey.ini
	fi
