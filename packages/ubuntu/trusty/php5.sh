...
if [[ $? == 0 && -d $halBox_Base/config/php5/ ]]; then
	cp -r $halBox_Base/config/php5/* /
	...
	for halBox_PHP_package in $halBox_PHP_packages; do
		echo -e "\e[1;32mDave, I'm installing '$halBox_PHP_package'.\e[0m"

		if [[ -f $halBox_Base/packages/$halBox_OS/$halBox_OS_Codename/$halBox_PHP_package.sh ]]; then
			source $halBox_Base/packages/$halBox_OS/$halBox_OS_Codename/$halBox_PHP_package.sh
		else
			apt-get -qq install $halBox_PHP_package > /dev/null 2>&1

			if [[ $? != 0 ]]; then
				echo -e "\e[1;31mSomething went wrong installing '$halBox_PHP_package'.\e[0m"
			fi
		fi

		if [[ -f /etc/php5/mods-available/${halBox_PHP_package:5}.ini ]]; then
			sed -i "s~^#~;~" /etc/php5/mods-available/${halBox_PHP_package:5}.ini && php5enmod ${halBox_PHP_package:5}
		fi
	done
	...
	echo -e "\e[1;31mSomething went wrong installing 'php5'.\e[0m"
fi
