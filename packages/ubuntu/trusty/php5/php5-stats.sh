...
if [[ ! -f /etc/php5/mods-available/stats.ini ]]; then
    ...
    pear config-set php_ini /etc/php5/mods-available/stats.ini > /dev/null && yes "" | pecl -q install -f stats > /dev/null

    if [[ $(cat /etc/php5/mods-available/stats.ini | wc -c) -le 1 ]]; then
        echo -e "\e[1;31mSomething went wrong installing '$halBox_PHP5_package'.\e[0m" && rm -f /etc/php5/mods-available/stats.ini
    else
        php5enmod stats
    fi
