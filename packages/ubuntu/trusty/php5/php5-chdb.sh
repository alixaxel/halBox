...
if [[ ! -f /etc/php5/mods-available/chdb.ini ]]; then
    ...
    pear config-set php_ini /etc/php5/mods-available/chdb.ini > /dev/null && yes "" | pecl -q install -f chdb > /dev/null

    if [[ $(cat /etc/php5/mods-available/chdb.ini | wc -c) -le 1 ]]; then
        echo -e "\e[1;31mSomething went wrong installing '$halBox_PHP5_package'.\e[0m" && rm -f /etc/php5/mods-available/chdb.ini
    else
        php5enmod chdb
    fi
