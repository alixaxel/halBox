...
if [[ ! -f /etc/php5/mods-available/uuid.ini ]]; then
	...
	pear config-set php_ini /etc/php5/mods-available/uuid.ini > /dev/null && printf "\n" | pecl -q install -f uuid > /dev/null

	if [[ $(cat /etc/php5/mods-available/uuid.ini | wc -c) -le 1 ]]; then
		echo -e "\e[1;31mSomething went wrong installing '$halBox_PHP_package'.\e[0m" && rm -f /etc/php5/mods-available/uuid.ini
	fi
